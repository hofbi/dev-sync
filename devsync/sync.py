    ...
    def sync(self, target, report):
        for element in self.__backup_folders:
            ...
            logger.debug('Running Rsync' +
                         ...
                         '\n\tTarget: ' + target.path +
                         '\n\tOptions: ' + options + '\n')
            subprocess.check_call('rsync {} {} {}'.format(options, element.path, target.path), shell=True,
                                  cwd=self.__root)
...
class RepoSync:
    ...
    def update_repos(self, target, last_update, report):
        ...
        logger.verbose('{} Repos to update on target {}\n'.format(len(all_repos), target.path))
    ...
    def get_all_repos(self):
        ...
        for element in self.__backup_folders:
            repos = element.repos
            logger.verbose('--> ' + str(len(repos)) + ' Repos found in path ' + element.path)
