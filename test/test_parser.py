...
class YMLConfigParserTest(unittest.TestCase):
    ...
    def test_get_yaml_content_valid_file_not_empty(self):
        self.assertIsNotNone(YMLConfigParser.get_yaml_content(YMLConfigParserTest.CONFIG))
    ...
    def test_parse_targets_valid_file_correct(self):
        self.assertEqual(2, len(YMLConfigParser.parse_targets(YMLConfigParserTest.SHORT_DEST)))

    @patch('devsync.parser.YMLConfigParser.get_yaml_content', MagicMock(return_value=[]))
    def test_parse_targets__empty_file_correct(self):
        self.assertEqual(0, len(YMLConfigParser.parse_targets(YMLConfigParserTest.SHORT_DEST)))

    def test_get_target_from_argument_is_short_dest(self):
        self.assertEqual('/tmp', YMLConfigParser.get_target_from_argument(YMLConfigParserTest.SHORT_DEST, 'TEST').path)

    def test_get_target_from_argument_regular_path(self):
        self.assertEqual('/tmp', YMLConfigParser.get_target_from_argument(YMLConfigParserTest.SHORT_DEST, '/tmp').path)
